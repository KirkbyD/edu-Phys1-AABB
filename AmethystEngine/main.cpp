...
int main() {
	...
	//sData subsystem, cmd, target, endpos, time;
	//subsystem.cmdStrData = "model";
	//cmd.cmdStrData = "moveto";
	//target.cmdStrData = "DESTROY";
	//endpos.cmdNumData = glm::vec4(150.0f, 100.0f, 50.0f, 1.0f);
	//time.cmdNum = 1000.0f;
	//
	//std::vector<sData> vecParams;
	//vecParams.push_back(subsystem);
	//vecParams.push_back(cmd);
	//vecParams.push_back(target);
	//vecParams.push_back(endpos);
	...
	while (!glfwWindowShouldClose(_window)) {
		...
		if (!isDevCon) {
			...
			if (mpLight.find("Tracker") != mpLight.end()) {
				glm::vec3 lightDirection = glm::normalize(g_vec_pComplexObjects[vpComplexObjectsCurrentPos]->getPosition() - mpLight["Tracker"]->getPosition());
				mpLight["Tracker"]->direction = glm::vec4(lightDirection, 1.0f);
			}
			{
				////Light deletion for collisions
				//std::vector<cLightObject*>lightsToDelete;
				//for (int i = 0; i != g_vec_pLightObjects.size(); i++) {
				//	cLightObject* theLight = g_vec_pLightObjects[i];
				//	if (theLight->friendlyName == "CollisionLight") {
				//		theLight->atten.y *= 1.2f;
				//		if (theLight->atten.y > 1.0f) {
				//			lightsToDelete.push_back(theLight);
				//		}
				//	}
				//}
				//if (!lightsToDelete.empty()) {
				//	for (unsigned int i = 0; i < lightsToDelete.size(); i++) {
				//		for (unsigned int j = 0; j != g_vec_pLightObjects.size(); j++) {
				//			if (lightsToDelete[i] == g_vec_pLightObjects[j]) {
				//				g_vec_pLightObjects.erase(g_vec_pLightObjects.begin() + (j));
				//				g_vec_pComplexObjects[0]->RemoveLight(lightsToDelete[i]);
				//				delete lightsToDelete[i];
				//				break;
				//			}
				//		}
				//	}
